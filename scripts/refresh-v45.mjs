...
const RAW_FILE = 'scripts/gate-live-2026-09-17-v45.tsv';
const REOPEN_FILE = 'scripts/reopen-live-2026-09-17-v45.tsv';
const REFRESH_INSTANT = new Date('2026-09-17T20:05:00Z');
...
if (!previousConfig.snapshotMetrics.refreshId.endsWith('-AUTHORITATIVE-v44')) fail(`expected v44 predecessor, got ${previousConfig.snapshotMetrics.refreshId}`);
...
const manifest = {
  ...
  dataSources: [
    {
      ...
      rawCapture: RAW_FILE,
      note: `Live read paged to exhaustion with a stable data.total (${openBucket.length} rows) at 2026-09-17T20:05:00Z; the ${gate.length} gate rows are retained verbatim in ${RAW_FILE} and the ${reopen.length} Reopen rows as ticket numbers in ${REOPEN_FILE}.`,
    },
    {
      name: 'Outlook',
      note: OUTLOOK.note,
  ...
  developerNotes: [
    `v45 live read: ${openBucket.length} open-system rows = ${statusCounts.New ?? 0} New / ${statusCounts.Pending ?? 0} Pending / ${reopen.length} Reopen; exact UFN New/Pending gate ${gate.length}.`,
...
readme = readme.replace(/### v43 -> v44 \(/, '### v43 -> v44 (superseded by v45) (');
if (/### v44 -> v45[\s\S]*?(?=### |$)/.test(readme)) readme = readme.replace(/### v44 -> v45[\s\S]*?(?=### |$)/, reconciliationNote);
