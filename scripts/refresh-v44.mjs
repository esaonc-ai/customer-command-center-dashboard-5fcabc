#!/usr/bin/env node
/**
 * NHT/Cesanek Customer Command Center refresh v44.
 *
 ...
 * paged to exhaustion on 2026-09-17 at 14:24:19Z) captured verbatim in
 * scripts/gate-live-2026-09-17-v44.tsv (the raw export is authoritative for this cycle).
 */
...
const RAW_FILE = 'scripts/gate-live-2026-09-17-v44.tsv';
const REFRESH_INSTANT = new Date('2026-09-17T14:24:19Z');
...
if (!previousConfig.snapshotMetrics.refreshId.endsWith('-AUTHORITATIVE-v43')) fail(`expected v43 predecessor, got ${previousConfig.snapshotMetrics.refreshId}`);
...
const manifest = {
  ...
  dataSources: [
    {
      ...
      rawCapture: RAW_FILE,
      note: `Live read paged to exhaustion with a stable data.total (${openBucket.length} rows) at 2026-09-17T14:24:19Z; raw rows retained in ${RAW_FILE}.`,
    },
    {
      name: 'Outlook',
      note: OUTLOOK.note,
  ...
  developerNotes: [
    `v44 live read: ${openBucket.length} open-system rows = ${statusCounts.New ?? 0} New / ${statusCounts.Pending ?? 0} Pending / ${reopen.length} Reopen; exact UFN New/Pending gate ${gate.length}.`,
...
readme = readme.replace(/### v42 -> v43 \(/, '### v42 -> v43 (superseded by v44) (');
if (/### v43 -> v44[\s\S]*?(?=### |$)/.test(readme)) readme = readme.replace(/### v43 -> v44[\s\S]*?(?=### |$)/, reconciliationNote);
