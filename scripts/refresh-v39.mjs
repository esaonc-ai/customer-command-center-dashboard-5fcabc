#!/usr/bin/env node
/**
 * v38 -> v39 refresh for the NHT/Cesanek Customer Command Center Dashboard.
 *
 * Authoritative input: live Ticket Ops gate re-read (dept 323826714354839552) captured in
 * scripts/gate-flags-2026-09-13-v39.csv — one row per open New/Pending ticket:
 *   ticketNumber|displayStatusName|closeFlag|isSlaBreached|isOverdue
 * plus the 9 arrivals captured in scripts/arrivals-v39.json.
 *
 ...
 *
 * This cycle HAS movement: +9 arrivals (UFN-70680..UFN-70688), 0 departures, 0 exclusion changes.
 */
...
const REFRESH_INSTANT = new Date('2026-09-13T15:16:00Z'); // 2026-09-13 11:16 ET

const EXPECTED = {
  gateTotal: 349,
  gateStatuses: { New: 283, Pending: 66 },
  openBucketTotal: 401,
  reopenTotal: 52,
  eligible: 320,
  ...
  arrivals: [
    'UFN-70688', 'UFN-70687', 'UFN-70686', 'UFN-70685', 'UFN-70684',
    'UFN-70683', 'UFN-70682', 'UFN-70681', 'UFN-70680',
  ],
};

const GATE_CSV = 'scripts/gate-flags-2026-09-13-v39.csv';
const ARRIVALS_JSON = 'scripts/arrivals-v39.json';
...
if (!previousRefreshId.endsWith('-AUTHORITATIVE-v38')) fail(`expected a v38 predecessor, got ${previousRefreshId}`);
...
if (JSON.stringify(arrivals) !== JSON.stringify([...EXPECTED.arrivals].sort())) {
  fail(`arrivals ${JSON.stringify(arrivals)} do not match the declared v39 arrivals ${JSON.stringify(EXPECTED.arrivals)}`);
}
if (departures.length) fail(`unexpected departures: ${JSON.stringify(departures)}`);
if (gate.size !== baselineGate.size + EXPECTED.arrivals.length) fail('gate set does not reconcile with the v38 baseline + declared arrivals');
...
/* per-record reconciliation against the v38 baseline: adopt live values, record movements */
const flagChanges = [];
for (const t of tickets) {
...
manifest.refresh = {
  ...
  previousRefreshId,
  status: 'complete',
  keyChange: `Fresh NHT/Cesanek Ticket Ops gate read at ${refreshTimestamp}: ${EXPECTED.openBucketTotal} open-system UFN rows (${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending at the gate, ${EXPECTED.reopenTotal} Reopen). ${EXPECTED.arrivals.length} arrivals and 0 departures; all ${billingIds.length} billing-family and ${duplicateChildren.length} conversation-duplicate exclusions still present. Working set is ${recomputed.length} conversations (${byStatus.New} New / ${byStatus.Pending} Pending).`,
};
manifest.developerNotes = [
  `v39 re-read the live New+Pending gate: ${EXPECTED.gateTotal} rows = ${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending inside a ${EXPECTED.openBucketTotal}-row open bucket that also holds ${EXPECTED.reopenTotal} Reopen rows (all system status 10, excluded by displayStatusName).`,
  `Movement this cycle: ${EXPECTED.arrivals.length} arrivals (${EXPECTED.arrivals.join(', ')}) against 0 departures. The eligible working set moved ${tickets.length} -> ${recomputed.length}.`,
  `The ${billingIds.length} billing / UF Billing / storage / handling invoice-family exclusions and the ${duplicateChildren.length} source-backed conversation duplicates were all re-confirmed present in the live gate. Deduplication still collapses only CASE/DN conversation identity, never subject text.`,
  ...
  'The UFN-67030 citation is stale and is NOT in this department\'s New/Pending or Reopen rows at this read; it is Solved on authoritative status (displayStatusSystemStatus 20, closedTime 09/01/2026). Its exclusion is driven by status, not by closeFlag.',
  ...
  'Outlook context remains supplemental and never contributes to ticket counts, queue, buckets, customer health or SLA metrics.',
];
manifest.dataSources = [
  ...
  {
    name: 'Outlook',
    note: 'Delegated NHT/Cesanek CS mailbox context read this cycle, supplemental and non-blocking.',
  },
];
...
manifest.excludedThisCycle = {
  ...
  departures: { count: 0, ticketNumbers: [] },
...
/* ---- README ----------------------------------------------------------------- */
const stateSection = `## Current Dashboard State (Last Refresh: ${etHeading} ET - AUTHORITATIVE v39)
...
| Arrivals this cycle | **+${EXPECTED.arrivals.length}** (${EXPECTED.arrivals.join(', ')}) &middot; departures **0** |
...
| Last Refresh | ${refreshTimestamp} (**AUTHORITATIVE v39** - live Ticket Ops gate read of department ${DEPARTMENT_ID}, independent of the prior cycle) |

`;

const reconciliationNote = `### v38 -> v39 (Sep 13 8:41 AM ET -> ${etHeading} ET)
...
- **Arrivals are exact and enumerated.** ${EXPECTED.arrivals.length} new tickets entered the gate and none departed: ${EXPECTED.arrivals.join(', ')}. They resolve to ${newArrivalCustomers.length} customer labels: ${newArrivalCustomers.join('; ')}. Nothing else in the gate moved in status.
...
const v39Pattern = /### v38 -> v39[\s\S]*?(?=### |$)/;
if (v39Pattern.test(readme)) readme = readme.replace(v39Pattern, reconciliationNote);
