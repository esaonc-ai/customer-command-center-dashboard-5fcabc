#!/usr/bin/env node
/**
 * v39 -> v40 refresh for the NHT/Cesanek Customer Command Center Dashboard.
 *
 * Authoritative input: live Ticket Ops gate re-read (dept 323826714354839552) captured in
 * scripts/gate-flags-2026-09-13-v40.csv — one row per open New/Pending ticket:
 *   ticketNumber,displayStatusName,closeFlag,isSlaBreached,isOverdue
 * plus the 4 arrivals captured in scripts/arrivals-v40.json.
 *
 ...
 *
 * This cycle HAS movement in both directions: +4 arrivals (UFN-70689, UFN-70692, UFN-70694, UFN-70703)
 * and -6 departures (UFN-70680..UFN-70685 left the gate). No per-record flag or status changes.
 */
...
const REFRESH_INSTANT = new Date('2026-09-13T21:05:00Z'); // 2026-09-13 17:05 ET

const EXPECTED = {
  gateTotal: 347,
  gateStatuses: { New: 281, Pending: 66 },
  openBucketTotal: 400,
  reopenTotal: 53,
  eligible: 318,
  closeFlagTrueGate: 20,
  closeFlagTrueEligible: 19,
  arrivals: ['UFN-70689', 'UFN-70692', 'UFN-70694', 'UFN-70703'],
  departures: ['UFN-70680', 'UFN-70681', 'UFN-70682', 'UFN-70683', 'UFN-70684', 'UFN-70685'],
};

const GATE_CSV = 'scripts/gate-flags-2026-09-13-v40.csv';
const ARRIVALS_JSON = 'scripts/arrivals-v40.json';
...
if (!previousRefreshId.endsWith('-AUTHORITATIVE-v39')) fail(`expected a v39 predecessor, got ${previousRefreshId}`);
...
if (JSON.stringify(arrivals) !== JSON.stringify([...EXPECTED.arrivals].sort())) {
  fail(`arrivals ${JSON.stringify(arrivals)} do not match the declared v40 arrivals ${JSON.stringify(EXPECTED.arrivals)}`);
}
if (JSON.stringify(departures) !== JSON.stringify([...EXPECTED.departures].sort())) {
  fail(`departures ${JSON.stringify(departures)} do not match the declared v40 departures ${JSON.stringify(EXPECTED.departures)}`);
}
if (gate.size !== baselineGate.size + EXPECTED.arrivals.length - EXPECTED.departures.length) {
  fail('gate set does not reconcile with the v39 baseline + declared arrivals - departures');
}
...
/* per-record reconciliation against the v39 baseline: adopt live values, record movements */
const flagChanges = [];
for (const t of tickets) {
...
for (const id of EXPECTED.departures) if (baselineEligibleIds.includes(id) === false) fail(`declared departure ${id} was not in the v39 eligible set`);
...
const departureCustomers = [...new Set(EXPECTED.departures.map((id) => tickets.find((t) => t.ticketId === id).customer))].sort();
...
manifest.refresh = {
  ...
  previousRefreshId,
  status: 'complete',
  keyChange: `Fresh NHT/Cesanek Ticket Ops gate read at ${refreshTimestamp}: ${EXPECTED.openBucketTotal} open-system UFN rows (${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending at the gate, ${EXPECTED.reopenTotal} Reopen). ${EXPECTED.arrivals.length} arrivals and ${EXPECTED.departures.length} departures; all ${billingIds.length} billing-family and ${duplicateChildren.length} conversation-duplicate exclusions still present. Working set is ${recomputed.length} conversations (${byStatus.New} New / ${byStatus.Pending} Pending).`,
};
manifest.developerNotes = [
  `v40 re-read the live New+Pending gate: ${EXPECTED.gateTotal} rows = ${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending inside a ${EXPECTED.openBucketTotal}-row open bucket that also holds ${EXPECTED.reopenTotal} Reopen rows (all system status 10, excluded by displayStatusName).`,
  `Movement this cycle: ${EXPECTED.arrivals.length} arrivals (${EXPECTED.arrivals.join(', ')}) against ${EXPECTED.departures.length} departures (${EXPECTED.departures.join(', ')}). The eligible working set moved ${tickets.length} -> ${recomputed.length}.`,
  `Departures are enumerated, not inferred: ${EXPECTED.departures.join(', ')} were all in the v39 gate and are absent from the live v40 gate. They are the 2026-09-13 Appointment Watch / Dropship daily-report batch that closed once its report cycle completed. Their leaving the gate is a status outcome in Ticket Ops, not an exclusion applied by this pipeline.`,
  `The ${billingIds.length} billing / UF Billing / storage / handling invoice-family exclusions and the ${duplicateChildren.length} source-backed conversation duplicates were all re-confirmed present in the live gate (24/24 and 5/5). Deduplication still collapses only CASE/DN conversation identity, never subject text. The exclusion set is unchanged this cycle; six of the 24 carry no billing wording in their titles, which is exactly why the explicit ticket-number list is used rather than a title heuristic.`,
  `${closeFlagTrueRetained} live closeFlag=true records remain in the eligible set (gate-wide ${closeFlagGate}, of which UFN-65196 is billing-excluded). closeFlag stays evidence-only and is never an eligibility gate; all ${closeFlagGate} sit on Pending rows that are system-OPEN.`,
  'The UFN-67030 citation remains stale: authoritative Ticket Ops status is Solved (displayStatusSystemStatus 20, closedTime 09/01/2026), so it is out of the gate on status, not on closeFlag. The rule stands and is better evidenced by the live closeFlag=true Pending rows (UFN-70443, UFN-70412, UFN-70304, UFN-70243, UFN-70130, UFN-69781).',
  ...
  'No Pending sub-status breakdown is published: all 66 Pending rows carry a single display status "Pending" (id 6).',
  ...
  'Outlook context was refreshed for this cycle and remains supplemental and non-blocking: it never contributes to ticket counts, queue, buckets, customer health or SLA metrics.',
];
...
outlookContext.mailboxReadWindow = 'Most recent UFN message page reviewed for this cycle; the fuller 2026-08-30 to 2026-09-13 thread inventory below is carried forward unchanged until superseded.';
outlookContext.sharedMailbox = {
  mailbox: 'nht.cs@unisco.com',
  readable: false,
  reason: 'Access denied (403, insufficient Mail.Read / Mail.Read.Shared) this cycle. Traffic to and from this shared box remains visible because it is copied into the delegated nicole.weber@unisco.com mailbox. Disclosed as a limitation rather than silently dropped.',
};
outlookContext.thisCycleRead = {
  messagesRetrieved: 25,
  distinctThreadsPostDedup: 13,
  threadsMappedToUfn: 13,
  ufnRefs: ['UFN-68515', 'UFN-70261', 'UFN-68506', 'UFN-69120', 'UFN-69103', 'UFN-69129', 'UFN-69781', 'UFN-67809', 'UFN-70542', 'UFN-70553', 'UFN-70252', 'UFN-70414', 'UFN-69116'],
...
outlookContext.coverage = {
  ...
  note: `Mailbox context is supplemental and non-blocking. ${outlookContext.threadsLinkedToEligibleTickets} of the ${outlookContext.threadsMatched} supplied UFN threads link to eligible conversations; Outlook never adds to Ticket Ops counts, queue, buckets, customer health or SLA metrics.`,
};
...
/* ---- README ----------------------------------------------------------------- */
const stateSection = `## Current Dashboard State (Last Refresh: ${etHeading} ET - AUTHORITATIVE v40)
...
| Arrivals this cycle | **+${EXPECTED.arrivals.length}** (${EXPECTED.arrivals.join(', ')}) &middot; departures **${EXPECTED.departures.length}** (${EXPECTED.departures.join(', ')}) |
...
| Last Refresh | ${refreshTimestamp} (**AUTHORITATIVE v40** - live Ticket Ops gate read of department ${DEPARTMENT_ID}, independent of the prior cycle) |

`;

const reconciliationNote = `### v39 -> v40 (Sep 13 11:16 AM ET -> ${etHeading} ET)

- **Net movement: -${EXPECTED.departures.length - EXPECTED.arrivals.length} eligible conversations.** The authoritative New+Pending gate moved ${EXPECTED.gateTotal - EXPECTED.arrivals.length + EXPECTED.departures.length} -> ${EXPECTED.gateTotal} rows (${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending) inside a ${EXPECTED.openBucketTotal}-row open bucket that still holds ${EXPECTED.reopenTotal} Reopen rows.
- **Arrivals are exact and enumerated.** ${EXPECTED.arrivals.length} tickets entered the gate: ${EXPECTED.arrivals.join(', ')}. They resolve to ${newArrivalCustomers.length} customer labels: ${newArrivalCustomers.join('; ')}.
- **Departures are exact and enumerated.** ${EXPECTED.departures.length} tickets left the gate: ${EXPECTED.departures.join(', ')} (${departureCustomers.join('; ')}). These are the 2026-09-13 Appointment Watch / Dropship daily-report series that closed on authoritative status once their report cycle completed - they were not removed by any exclusion rule.
- **No flag or status drift on carried-forward records.** A per-record comparison of opsStatus, closeFlag, isSlaBreached and isOverdue across every v39 eligible ticket still present in the v40 gate returned ${flagChanges.length} differences.
- **Exclusions re-confirmed against the live gate, not re-guessed.** All ${billingIds.length} billing-family rows (24/24) and all ${duplicateChildren.length} duplicate conversations (5/5) were present in the live read, so the exclusion set is unchanged. UFN-60009 (Amazon Invoice reference on a BOL request) and UFN-69231 (ODFL PRO-number request) remain retained pending a business ruling.
- **closeFlag evidence unchanged and re-proven.** ${closeFlagTrueRetained} live \\\\\\\`closeFlag=true\\\\\\\` records remain in the eligible set (gate-wide ${closeFlagGate}; UFN-65196 is billing-excluded). All ${closeFlagGate} sit on Pending rows that are system-OPEN, which is exactly the auto-close artifact that makes closeFlag unusable as an eligibility gate.
...
readme = readme.replace('### v38 -> v39', '### v38 -> v39 (superseded by v40)');
const v40Pattern = /### v39 -> v40[\s\S]*?(?=### |$)/;
...
console.log(JSON.stringify({
  ...
  arrivals, departures, flagChanges: flagChanges.length,
