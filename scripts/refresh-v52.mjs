...
//   open-system bucket 349 = 241 New / 70 Pending / 38 Reopen   (identical to v51)
//   New+Pending gate 311 rows                                   (identical to v51: 0 arrivals / 0 departures)
...
const REFERENCE = argOf('--reference') || '2026-09-20T20:30:00-04:00';
const WRITE = args.includes('--write');

const VERSION = 'v52';
const REFRESH_ID = `refresh-${REFERENCE}-AUTHORITATIVE-${VERSION}`;
const PREVIOUS_REFRESH_ID = 'refresh-2026-09-20T19:50:00-04:00-AUTHORITATIVE-v51';
const REFERENCE_TIME_ET = 'Sep 20 8:30 PM ET';
const DEPARTMENT_ID = '323826714354839552';
const READ_TIMESTAMP_UTC = '2026-09-21T00:30:00Z';
...
/* 0. gate identity guard: v52 gate must be set-identical to the persisted v51 capture */
...
const gate = readFileSync('scripts/gate-live-2026-09-21-v52.txt', 'utf8').trim().split(/\s+/);
...
const prevGate = readFileSync('scripts/gate-live-2026-09-20-v51.txt', 'utf8').trim().split(/\s+/);
if (prevGate.length !== GATE_TOTAL) throw new Error(`v51 gate capture must hold ${GATE_TOTAL} rows, got ${prevGate.length}`);
...
console.log(`gate identity OK: ${GATE_TOTAL} rows, 0 arrivals / 0 departures vs v51`);
...
manifest.refresh = {
  ...
  keyChange:
    ...
    `(${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen) and the same ${GATE_TOTAL}-row New/Pending gate as v51. ` +
...
manifest.dataSources = [
  {
    ...
    rawCapture: 'scripts/gate-live-2026-09-21-v52.txt (all 311 gate ticket numbers, set-identical to v51)',
    note:
      ...
      `to the persisted v51 capture - 0 arrivals, 0 departures.`,
  },
  {
    name: 'Outlook',
    note:
...
manifest.developerNotes = [
  `v52 live read: ${OPEN_BUCKET} open-system rows = ${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen; exact UFN New/Pending gate ${GATE_TOTAL} (no movement vs v51).`,
  `Movement is verified, not asserted: the ${GATE_TOTAL} gate rows are set-compared against the persisted v51 capture (scripts/gate-live-2026-09-20-v51.txt). Exactly 0 arrivals and 0 departures; refresh-v52.mjs aborts on any diff.`,
  `One transcription gap was investigated and closed. The delegate's manual gate transcription returned 310 of the 311 rows and omitted UFN-35588. UFN-35588 was re-read live via /v1/iam/tickets/brief/number/UFN-35588 and reads displayStatusName "Pending" / displayStatusSystemStatus 10 (OPEN) / closeFlag false - it never left the gate. The missing row was a transcription artifact, not a departure.`,
  `The newest ticket in the live read is still UFN-71236 (created 2026-09-20T16:01:37 local), the same newest arrival as v51, and page 1 returns the identical newest-200 rows - so no new UFN ticket entered the department since the v51 read.`,
  ...
  `SLA/workload: rows retain their stored isSlaBreached/isOverdue flags (v51 methodology, no arrivals this cycle). A dueDate-derived cross-check flags ${dueDerivedBreached} rows as past due vs ${breachedList.length} on stored flags - disclosed, not silently substituted.`,
...
manifest.reconciliation = {
  ...
  eligibleDepartures: [],
  note:
    `Verified against the persisted v51 capture: the ${GATE_TOTAL}-row gate is set-identical, so the eligible set stays at ${total}.`,
};
manifest.verifiedAgainst = `Live Ticket Ops read ${READ_TIMESTAMP_UTC} (department ${DEPARTMENT_ID}, displayStatusSystemStatus=10), set-verified against scripts/gate-live-2026-09-21-v52.txt and the v51 capture`;
...
const stateSection = `## Current Dashboard State (Last Refresh: ${REFERENCE_TIME_ET} - AUTHORITATIVE v52)
...
| Eligible | **${total}** conversations (${byStatus.New} New, ${byStatus.Open} Open, ${byStatus.Pending} Pending) - 0 arrivals / 0 departures vs v51 | 
...
| Last Refresh | ${REFERENCE} (**AUTHORITATIVE v52**, department ${DEPARTMENT_ID}) |

`;
readme = readme.replace(/## Current Dashboard State \(Last Refresh:[\s\S]*?(?=## Developer Reconciliation Note)/, stateSection);

const reconciliationNote = `### v51 -> v52 (Sep 20 7:50 PM ET -> ${REFERENCE_TIME_ET})

- **Net movement: 0 eligible conversations.** The live Ticket Ops read returns the same ${OPEN_BUCKET} system-open rows (${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen) and the same ${GATE_TOTAL}-row New/Pending gate as v51. The eligible set is unchanged at **${total}** (${byStatus.New} New / ${byStatus.Pending} Pending). Nothing arrived and nothing departed.
- **Movement is verified, not asserted.** The ${GATE_TOTAL}-row gate was captured to scripts/gate-live-2026-09-21-v52.txt and set-compared against the persisted v51 capture: exact set identity, 0 arrivals / 0 departures. scripts/refresh-v52.mjs aborts on any diff.
...
console.log(`\nWROTE v52 artifacts: tickets.json, refresh-manifest.json, config.json (x3), outlook-context.json (x2), structured_list.json, README.md`);
