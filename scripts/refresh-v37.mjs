#!/usr/bin/env node
/**
 * v36 -> v37 refresh for the NHT/Cesanek Customer Command Center Dashboard.
 *
 * Authoritative input: live Ticket Ops gate re-read (dept 323826714354839552) captured in
 * scripts/gate-flags-2026-09-13-v37.csv — one row per open New/Pending ticket:
 ...
 *
 * The script refuses to write unless the live gate reconciles with the v36 baseline:
 *   baseline gate set (310 eligible + 24 billing + 5 duplicate conversations = 339) plus the single
 *   arrival UFN-70676 = 340, and per-record status/closeFlag/SLA/overdue match the baseline.
 */
...
const REFRESH_INSTANT = new Date('2026-09-13T10:36:00Z'); // 2026-09-13 06:36 ET
...
// Authoritative record for the single arrival this cycle (live Ticket Ops read; corroborated by an
// independent paged read of the same department in the same window).
const ARRIVAL = {
  ticketId: 'UFN-70676',
  customer: 'UPTIME ENERGY INC',
  customerEmail: 'mary.smothers@unisco.com',
  opsStatus: 'New',
  displayStatusName: 'New',
  displayStatusSystemStatus: 10,
  priority: 'Medium',
  priorityNameSource: 'ticket',
  prioritySourceMissing: false,
  subject: 'Re: Ecommerce Unis',
  createdDate: '2026-09-13',
  createdAt: '2026-09-13T05:22:57.000Z',
  lastUpdated: '2026-09-13',
  updatedAt: '2026-09-13T05:23:04.000Z',
  dueDate: '2026-09-16T21:00:00.000Z',
  assigned: 'Unassigned',
  closeFlag: false,
  slaStatus: 'On Track',
  isOverdue: false,
...
const csvLines = (await readFile('scripts/gate-flags-2026-09-13-v37.csv', 'utf8')).trim().split('\n');
...
if (!previousRefreshId.endsWith('-AUTHORITATIVE-v36')) fail(`expected a v36 predecessor, got ${previousRefreshId}`);
...
const expectedGate = new Set([...baselineGate, ARRIVAL.ticketId]);
const arrivals = [...gate.keys()].filter((id) => !baselineGate.has(id));
const departures = [...baselineGate].filter((id) => !gate.has(id));
if (arrivals.length !== 1 || arrivals[0] !== ARRIVAL.ticketId) fail(`unexpected arrivals: ${JSON.stringify(arrivals)}`);
if (departures.length) fail(`unexpected departures: ${JSON.stringify(departures)}`);
if (gate.size !== expectedGate.size) fail('gate set does not reconcile with baseline + arrival');
...
/* per-record reconciliation against the v36 baseline (status / closeFlag / SLA / overdue) */
...
if (flagMismatches.length) fail(`v36 -> v37 record flags moved (${flagMismatches.length}): ${flagMismatches.join('; ')}`);
...
for (const [ticketId, live] of gate) {
  ...
  if (ticketId === ARRIVAL.ticketId) { refreshed.push({ ...ARRIVAL }); continue; }
...
manifest.refresh = {
  ...
  previousRefreshId,
  status: 'complete',
  keyChange: `Fresh NHT/Cesanek Ticket Ops gate read at ${refreshTimestamp}: ${EXPECTED.openBucketTotal} open-system UFN rows (${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending at the gate, ${EXPECTED.reopenTotal} Reopen). One arrival (${ARRIVAL.ticketId}) and no departures; all ${billingIds.length} billing-family and ${duplicateChildren.length} conversation-duplicate exclusions still present. Working set is ${recomputed.length} conversations (${byStatus.New} New / ${byStatus.Pending} Pending).`,
};
manifest.developerNotes = [
  `v37 re-read the live New+Pending gate: ${EXPECTED.gateTotal} rows = ${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending inside a ${EXPECTED.openBucketTotal}-row open bucket that also holds ${EXPECTED.reopenTotal} Reopen rows (all system status 10, excluded by displayStatusName).`,
  `Exactly one arrival this cycle: ${ARRIVAL.ticketId} ("${ARRIVAL.subject}", ${ARRIVAL.customer}, created ${ARRIVAL.createdAt}), non-billing, no CASE/DN identity collision. No previously eligible record left the gate and no per-record status, closeFlag, SLA or overdue flag moved against v36.`,
  ...
  'UFN-67030 was re-verified by ticket number this cycle: displayStatusName "Solved", displayStatusId 2, displayStatusSystemStatus 20, closeFlag=true, last updated 2026-09-01 16:57. It is closed on authoritative status, not on closeFlag; the closeFlag rule is unchanged and the example citation should be replaced with a live closeFlag=true Pending ticket such as UFN-70443 or UFN-70243.',
  ...
  'Outlook context was refreshed for this cycle and remains supplemental: 25 supplied UFN threads (13 re-confirmed in this cycle\'s most-recent window), 2 linking to eligible conversations. The delegated nht.cs@unisco.com shared mailbox could not be read this cycle (HTTP 403); nicole.weber@unisco.com was readable and is the mailbox scope actually used. Outlook never contributes to ticket counts, queue, buckets, customer health or SLA metrics.',
];
manifest.dataSources = [
  {
    ...
    note: `Authoritative New+Pending gate read at ${refreshTimestamp} (delegated Ticket Ops read of department ${DEPARTMENT_ID}, paged to exhaustion) plus an independent paged read of the same department in the same window; both agreed on the ${EXPECTED.gateTotal}-row gate.`,
  },
  {
    name: 'Ticket Ops Reopen reconciliation',
    endpoint: 'POST /v1/iam/tickets/page',
    query: { page: 'P', size: 200, input: { departmentIds: [DEPARTMENT_ID], displayStatusIds: ['1'] } },
    note: `Read-only reconciliation returned ${EXPECTED.reopenTotal} Reopen rows, all displayStatusSystemStatus 10; excluded from the eligible set by displayStatusName, not by closeFlag.`,
  },
  {
    name: 'Outlook',
    note: 'Delegated NHT/Cesanek CS mailbox context read this cycle (nicole.weber@unisco.com; nht.cs@unisco.com returned access denied), supplemental and non-blocking.',
  },
];
...
outlookContext.mailboxReadWindow = 'Most recent UFN message page (25 messages, receivedDateTime desc) reviewed for this cycle; the fuller 2026-08-30 to 2026-09-13 thread inventory below is carried forward unchanged.';
...
outlookContext.activeEscalations = [
  'Colavita / Vital (UFN-70252): customer chasing loading status; carrier escalated cancelled loads and threatened TONU.',
  'Hint Beverage / Fusion Transport (UFN-69781): HIGH IMPORTANCE - EDI tenders missing, loads at risk; thread links to an eligible conversation.',
  'KD Nutra / Pharmavite series (UFN-68506, UFN-68515, UFN-69103, UFN-69120): time-sensitive appointment/relabeling confirmations; none currently eligible.',
];
outlookContext.eligibleEscalations = 1;
outlookContext.dedupNotes = [
  ...(outlookContext.dedupNotes ?? []).filter((n) => !n.startsWith('Action/ask text was not supplied')),
...
/* ---- structured_list.json (was left at v34; regenerated here) ---------------- */
...
/* ---- README ----------------------------------------------------------------- */
const stateSection = `## Current Dashboard State (Last Refresh: ${etHeading} ET - AUTHORITATIVE v37)
...
| Last Refresh | ${refreshTimestamp} (**AUTHORITATIVE v37** - live Ticket Ops gate read of department ${DEPARTMENT_ID}, independent of the prior cycle) |

`;

const reconciliationNote = `### v36 -> v37 (Sep 13 12:33 AM ET -> ${etHeading} ET)

- **Net movement: +1.** The authoritative New+Pending gate moved from 339 to ${EXPECTED.gateTotal} rows (${EXPECTED.gateStatuses.New} New / ${EXPECTED.gateStatuses.Pending} Pending) inside a ${EXPECTED.openBucketTotal}-row open bucket that still holds ${EXPECTED.reopenTotal} Reopen rows. One arrival (${ARRIVAL.ticketId}, "${ARRIVAL.subject}", ${ARRIVAL.customer}, created ${ARRIVAL.createdAt}) and **zero departures**; per-record status, closeFlag, SLA and overdue flags matched v36 on all ${tickets.length} previously eligible records.
- **Exclusions re-confirmed against the live gate, not re-guessed.** All ${billingIds.length} billing-family rows and all ${duplicateChildren.length} duplicate conversations were present in the live read, so the exclusion set is unchanged. No new CASE/DN identity collision arrived with ${ARRIVAL.ticketId}.
- **closeFlag evidence unchanged.** ${closeFlagTrueRetained} live \`closeFlag=true\` records remain in the eligible set (gate-wide ${closeFlagGate}; UFN-65196 is billing-excluded). \`closeFlag\` is evidence only.
...
- **Flagged records remain retained.** The ${candidateOverlaps.length} same-load C.H. Robinson pairs and UFN-69231 / UFN-60009 are still carried unchanged pending a business ruling; deduplication continues to collapse only CASE/DN conversation identity.
...
const v37Pattern = /### v36 -> v37[\s\S]*?(?=### |$)/;
if (v37Pattern.test(readme)) readme = readme.replace(v37Pattern, reconciliationNote);
