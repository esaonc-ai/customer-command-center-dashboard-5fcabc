#!/usr/bin/env node
/**
 * NHT/Cesanek Customer Command Center refresh v45.
 *
 ...
 * paged to exhaustion on 2026-09-17 at 20:05:00Z) captured verbatim in
 * scripts/gate-live-2026-09-17-v45.tsv (the raw export is authoritative for this cycle).
 * The gate export carries the 292 New/Pending rows; the 43 Reopen rows are captured as
 * ticket numbers only in scripts/reopen-live-2026-09-17-v45.tsv.
 */
...
const RAW_FILE = 'scripts/gate-live-2026-09-18-v46.tsv';
const REOPEN_FILE = 'scripts/reopen-live-2026-09-18-v46.tsv';
const REFRESH_INSTANT = new Date('2026-09-18T09:35:00Z');
...
const OUTLOOK = {
  ...
  note: 'No Outlook read was possible this cycle either (the delegated-mailbox read returned no result); the last observed v42 values are retained for context only and were not re-verified.',
};
...
function conversationIdentity(title) {
  ...
  const dnMatch = value.match(/^\s*DN[-\s]?(\d{5,})\b/i);
...
function loadReopenRows() {
  const text = readText(REOPEN_FILE);
  const numbers = text.split('\n').map((line) => line.trim()).filter((line) => line !== '' && line !== 'ticketNumber');
  if (numbers.length === 0) fail('reopen capture is empty');
  if (numbers.some((number) => !number.startsWith('UFN-'))) fail('reopen capture contains a non-UFN row');
  if (new Set(numbers).size !== numbers.length) fail('reopen capture contains duplicate ticket numbers');
  return numbers.map((ticketNumber) => ({
    ticketNumber,
    title: '(reopen row captured as ticket number only)',
    displayStatusName: 'Reopen',
    displayStatusSystemStatus: 10,
    closeFlag: false,
    isSlaBreached: false,
    isOverdue: false,
...
if (!previousConfig.snapshotMetrics.refreshId.endsWith('-AUTHORITATIVE-v45')) fail(`expected v44 predecessor, got ${previousConfig.snapshotMetrics.refreshId}`);

const gateRows = loadBucket();
const reopenRows = loadReopenRows();
const openBucket = [...gateRows, ...reopenRows];
...
const reconciledOpenCount = (statusCounts.New ?? 0) + (statusCounts.Pending ?? 0) + reopen.length;
if (openBucket.length !== reconciledOpenCount) {
  fail(`open bucket reconciliation failed: ${openBucket.length} != ${reconciledOpenCount} New + Pending + Reopen`);
}
if (gate.length !== gateBeforeUfn.length) fail(`New/Pending gate contains ${gateBeforeUfn.length - gate.length} non-UFN rows`);
if (gate.some((record) => !['New', 'Pending'].includes(record.displayStatusName))) fail('gate contains a row outside New/Pending');
...
for (const [identity, group] of identityGroups) {
  ...
  for (const record of group.slice(1)) duplicateConversations.push({
    ticketId: record.ticketNumber,
    canonical: group[0].ticketNumber,
    conversationId: identity.replace(/^CASE-/, ''),
    reason: `excluded as an overlapping ${identity.startsWith('CASE-') ? 'CASE' : 'DN'} conversation; ${group[0].ticketNumber} is the earliest surviving row for ${identity}`,
  });
}
...
if (tickets.length === 0) fail('eligible ticket set is empty');
if (new Set(tickets.map((ticket) => ticket.ticketId)).size !== tickets.length) fail('eligible ticket numbers are not unique');
if (tickets.some((ticket) => !['New', 'Pending'].includes(ticket.displayStatusName))) fail('eligible set contains a row outside New/Pending');
if (tickets.some((ticket, index) => index > 0 && tickets[index - 1].createdAt.localeCompare(ticket.createdAt) < 0)) {
  fail('eligible ticket set is not sorted by createdAt descending');
}
...
const eligibleCustomerLabels = new Set(tickets.map((ticket) => ticket.customer));
const healthCustomerLabels = new Set(Object.keys(customerHealth.customers));
if (eligibleCustomerLabels.size !== healthCustomerLabels.size || [...eligibleCustomerLabels].some((label) => !healthCustomerLabels.has(label))) {
  fail('Customer Health does not cover every eligible customer label');
}
...
const manifest = {
  ...
  dataSources: [
    {
      ...
      rawCapture: RAW_FILE,
      note: `Live read paged to exhaustion with a stable data.total (${openBucket.length} rows) at 2026-09-18T09:35:00Z; the ${gate.length} gate rows are retained verbatim in ${RAW_FILE} and the ${reopen.length} Reopen rows as ticket numbers in ${REOPEN_FILE}.`,
    },
    {
      name: 'Outlook',
      note: OUTLOOK.note,
  ...
  developerNotes: [
    `v46 live read: ${openBucket.length} open-system rows = ${statusCounts.New ?? 0} New / ${statusCounts.Pending ?? 0} Pending / ${reopen.length} Reopen; exact UFN New/Pending gate ${gate.length}.`,
    ...
    `Outlook was unavailable again this cycle (the delegated-mailbox read returned no result); the v42 values (${OUTLOOK.messages} messages / ${OUTLOOK.threads} threads, latest ${OUTLOOK.lastObservedUtc}) are carried forward and labelled stale. They were not used in any ticket, queue, bucket, health or SLA metric.`,
...
readme = readme.replace(/### v44 -> v45 \(/, '### v44 -> v45 (superseded by v46) (');
if (/### v45 -> v46[\s\S]*?(?=### |$)/.test(readme)) readme = readme.replace(/### v45 -> v46[\s\S]*?(?=### |$)/, reconciliationNote);
