...
## Current Dashboard State (Last Refresh: Sep 7 22:53 ET – AUTHORITATIVE v24)

| Total Raw (open UFN, dept-gate) | 383 |
| Eligible | **311** (242 New, 0 Open, 69 Pending) |
| UFN-Count | 311 |
| Excluded | 72 — 41 Reopen display-name (outside New/Pending gate) + 22 billing/UF Billing/storage/handling/claims + 9 duplicate threads (same v23 audited sets, re-verified unchanged) |
...
| Customers | **55** distinct orgs (ticket-visible coverage; roster supplemental) |
| Priority | 311 Medium |
| SLA Risk | **ELEVATED** – 200 SLA-breached / 111 on-track (carried; no state change) |
| Action Buckets | Immediate **24** / Short-Term **35** / Medium-Term **70** / Watch **182** (+3h age advance: 17 rows Short→Medium, 2 rows Medium→Watch vs v23 24/52/55/180) |
| Outlook Coverage | Shared mailbox nht.cs@unisco.com **reachable this cycle** (delegated Graph; prior 403 cleared); window to 02:44Z — **0 new unique eligible threads** — **10** eligible-ticket threads carried forward (3.2%); 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident) |
| Last Refresh | 2026-09-07 22:53 ET (**AUTHORITATIVE** – fresh TicketOps LIVE pull ~02:53Z; server total 383 open UFN = 269 New/73 Pending/41 Reopen re-verified via displayStatusIds [11]/[6]; population identical to v23 19:38 ET — newest UFN-70035 == v23 max ⇒ 0 arrivals / 0 removals / 0 status flips; eligible **311** = 242 New/69 Pending (net 0); closeFlag still NOT a gate (19 retained); UFN-67030 remains closed 09/01 → outside gate) |
| Next Refresh | Hourly dashboard refresh continues; next daily summary email 2026-09-08T08:00:00-04:00 |

### ⏱ Queue Delta (this refresh: v23 → v24)

**AUTHORITATIVE v24 pull (2026-09-07 ~22:53 ET / 02:53Z)** — fresh TicketOps LIVE enumeration (dept 323826714354839552, `POST /v1/iam/tickets/page`, `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10], displayStatusIds:[11,6]}`): server total **383 open UFN** = **New 269** (displayStatusId 11 re-verified) + **Pending 73** (id 6 re-verified) + **Reopen 41** — distribution **identical to v23** (23:38Z). Eligible base (New+Pending) **342** = same set as v23 (newest open UFN-70035, created 09/07 21:36Z, == v23 max ⇒ **0 arrivals / 0 removals / 0 status flips** in the ~3h15m window). Audits carried unchanged: **41 Reopen** excluded (display name, outside gate); **22 billing/UF Billing/storage/handling/claims invoice items** (v23 audit ids unchanged, operational keepers retained); **9 duplicate threads** (same v23 pairs). → Final eligible **311 = 242 New / 0 Open / 69 Pending** (net 0 vs v23). Per-row ages advanced **+3 whole hours** from v23 anchor 2026-09-07T23:38:00Z (measured delta 3h15m; ageDays=floor(ageHours/24); **30 rows +1 age-day**); oldest UFN-33604 4473h→**4476h / 186d** (ageDays unchanged). Buckets recomputed **Immediate 24 / Short-Term 35 / Medium-Term 70 / Watch 182** (+3h moved 17 rows Short→Medium, 2 rows Medium→Watch). SLA 200 breached / 111 on track (carried — no state change among continuing rows); unassigned 253 (carried); customers 55 (tiers recomputed deterministic **37 Critical / 10 Warning / 8 Healthy** — unchanged). **UFN-67030** remains closed (09/01) → outside gate. **Outlook**: shared mailbox nht.cs@unisco.com **reachable** via delegated Graph this cycle (prior 403 cleared); scan window to 2026-09-08T02:44:00Z returned **0 brand-new unique eligible threads** — 10 eligible-ticket threads carried unchanged (3.2% of 311); 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident). All public/data synced.
...
**This refresh (Sep 7 22:53 ET) is an AUTHORITATIVE refresh (v24)** with a fresh TicketOps LIVE connection (dept 323826714354839552, `POST /v1/iam/tickets/page` with `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10], displayStatusIds:[11,6]}`; displayStatus counts re-verified server-side via `displayStatusIds [11]`=269 and `[6]`=73). All statuses verified directly against TicketOps; the open population is **383 = 269 New / 73 Pending / 41 Reopen** (identical to v23 — no tickets opened/closed/status-changed in the ~3h15m window; newest open UFN-70035 == v23 max). The eligibility gate is `displayStatusSystemStatus=open` + `displayStatusName ∈ {New,Pending}`; `closeFlag` is not a filter (19 live `closeFlag=true` tickets retained in the eligible set — auto-close artifacts must not cause false negatives). Billing/UF Billing/storage/handling/claims invoice items and duplicate ticket/email threads are excluded per the audited rule set in `refresh-manifest.json` (22 invoice items + 9 duplicate threads, unchanged v23 audit sets).

**Queue health**: 311 verified eligible tickets (242 New / 69 Pending) — identical set to v23 (0 new arrivals / 0 removals in the window; status distribution verified identical at the server-count level; SLA and assignment counts carried). 200 SLA-breached / 111 on-track; 253 unassigned; oldest 186d (UFN-33604, 4476h). Stale backlog remains dominated by recurring New automated/system reports (dropship EOD, appointment-watch, Amazon EDI alerts, Advanced Report – Dock Activity) and older New tickets awaiting assignment. Per-row ages advanced +3 whole hours from the v23 anchor (23:38Z → 22:53 ET); 30 rows advanced +1 age-day; action buckets recomputed to Immediate 24 / Short-Term 35 / Medium-Term 70 / Watch 182. Outlook enrichment: shared mailbox nht.cs@unisco.com reachable this cycle (delegated Graph, prior 403 cleared); 0 new unique eligible threads in window to 2026-09-08T02:44:00Z — 10 active eligible-ticket threads carried forward; non-blocking. Data is authoritative in `dashboard/data/` and `public/data/`.
