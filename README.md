...
## Current Dashboard State (Last Refresh: Sep 6 08:19 ET – AUTHORITATIVE v18)
...
| Outlook Coverage | Unavailable this cycle (shared mailbox 403 ErrorAccessDenied; delegated inbox scanned, 0 new hits; non-blocking) — **9** eligible-ticket threads carried forward (~3.3%); strongest signals Diageo/Ritual claim 18050 (UFN-69811), Boundless Walmart load-45872185 (UFN-69618/UFN-69771) |
| Last Refresh | 2026-09-06 08:19 ET (**AUTHORITATIVE** – fresh TicketOps LIVE pull; population verified IDENTICAL to v17 (07:28 ET); closeFlag still NOT a gate (19 retained); UFN-67030 remains Solved/system-20, closed 09/01 → outside gate) |
| Next Refresh | ~08:00 ET (daily summary email; next scheduled task 2026-09-07T08:00:00-04:00) |

### ⏱ Queue Delta (this refresh: v17 → v18)

**AUTHORITATIVE v18 pull (2026-09-06 ~08:19 ET)** — fresh TicketOps LIVE enumeration (dept 323826714354839552, `POST /v1/iam/tickets/page`, `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10]}`, UFN prefix client-side): **333 open UFN** (223 New / 73 Pending / 37 Reopen) — population and display-status distribution **IDENTICAL to the v17 07:28 ET pull** (server status-statistics verified; newest UFN-69916 created 09-05 21:02Z / 17:02 ET unchanged; oldest UFN-33604). Eligibility gate unchanged: `displayStatusSystemStatus=open` + `displayStatusName ∈ {New,Pending}`; `closeFlag` is **NOT** a filter → **19 live `closeFlag=true` tickets retained in the eligible set** (UFN-68733/UFN-68750 auto-close artifacts; UFN-69797 Diageo Picks, UFN-69618 Boundless no-labels, UFN-43887 Ritual stickering, UFN-59720). After the same audit exclusions — **37 Reopen**, **22 billing/UF Billing/storage/handling/claims invoice items** (all 22 v17 audit ids re-verified present in the raw open set; operational keepers retained: UFN-60009 BOL, UFN-68749 storage-SQFT data request, UFN-69818 open-RN ops digest, UFN-69231/UFN-63762/UFN-63959 invoice-dispute ops, UFN-69811 Diageo claim 18050, Boundless incident UFN-69771/UFN-69618/UFN-69760) and **3 duplicate ticket/email threads** (kept UFN-69896 dropped 69893; kept UFN-69892 dropped 69889; kept UFN-69663 dropped 69661) — **271 eligible = 202 New / 69 Pending**, an identical set to v17 (net 0: nothing opened/closed/status-changed in the ~51-min window). Per-row authoritative refresh anchored at 08:19 ET: ages recomputed from live created timestamps (ageHours/ageDays floored to the 08:19 ET pull anchor); 0 status / 0 assignment / 0 SLA changes; 0 createdDate/lastUpdated changes vs v17 file. SLA 200 breached / 71 on-track; unassigned 219; oldest 184d; customers 53; buckets Immediate 18 / Short-Term 46 / Medium-Term 30 / Watch 177 — all identical to v17. **UFN-67030** verified absent from the open set (remains Solved/system-20, closed 09/01) → outside gate. **Outlook**: delegated mailbox accessible via Graph but shared mailbox nht.cs@unisco.com still 403 ErrorAccessDenied; window 09-06 07:28 ET → 08:19 ET: 0 UFN/Cesanek hits (newest mailbox item remains 09-05 20:57Z VITA COCO transfer, unrelated) — non-blocking, 9 eligible-ticket threads carried forward unchanged; 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident). Watermark advanced to 2026-09-06T12:19:00Z (08:19 ET).
...
**This refresh (Sep 6 08:19 ET) is an AUTHORITATIVE refresh (v18)** with a fresh TicketOps LIVE connection (dept 323826714354839552, `POST /v1/iam/tickets/page` with `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10]}`, UFN prefix applied client-side). All statuses verified directly against TicketOps; the open population (333 = 223 New / 73 Pending / 37 Reopen) is verified IDENTICAL to the v17 07:28 ET pull (server status-statistics confirmed). The eligibility gate is `displayStatusSystemStatus=open` + `displayStatusName ∈ {New,Pending}`; `closeFlag` is not a filter (19 live `closeFlag=true` tickets retained in the eligible set — auto-close artifacts must not cause false negatives). Billing/UF Billing/storage/handling/claims invoice items and duplicate ticket/email threads are excluded per the audited rule set in `refresh-manifest.json`.

**Queue health**: 271 verified eligible tickets (202 New / 69 Pending) — same set as v17. 200 SLA-breached / 71 on-track; 219 unassigned; oldest 184d. Stale backlog is dominated by recurring New automated/system reports (dropship EOD, appointment-watch, Amazon Freight LTL alerts, Advanced Report – Dock Activity) and older New tickets awaiting assignment. Per-row ages recomputed from live created timestamps anchored at the 08:19 ET pull; 0 status / 0 assignment / 0 SLA changes vs v17. Outlook enrichment was unavailable this cycle (shared mailbox 403 ErrorAccessDenied; delegated inbox scanned 07:28→08:19 ET with 0 UFN/Cesanek hits; non-blocking) — 9 active eligible-ticket threads carried forward unchanged; watermark advanced to 08:19 ET. Data is authoritative in `dashboard/data/` and `public/data/`.
