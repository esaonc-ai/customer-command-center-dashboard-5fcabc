...
## Current Dashboard State (Last Refresh: Sep 8 03:20 ET – AUTHORITATIVE v25)

| Total Raw (open UFN, dept-gate) | 397 |
| Eligible | **325** (256 New, 0 Open, 69 Pending) |
| UFN-Count | 325 |
| Excluded | 72 — 41 Reopen display-name (outside New/Pending gate) + 22 billing/UF Billing/storage/handling/claims + 9 duplicate threads (same v24 audited sets, re-verified unchanged) |
...
| Customers | **55** distinct orgs (ticket-visible coverage; roster supplemental; +14 SMEG USA INC arrivals — no new orgs) |
| Priority | 325 Medium |
| SLA Risk | **ELEVATED** – 200 SLA-breached / 125 on-track (200 carried among 311 continuing + 14 new arrivals On Track) |
| Action Buckets | Immediate **38** / Short-Term **35** / Medium-Term **70** / Watch **182** (+4h age advance: 2 rows +1 age-day; all 14 new arrivals <24h in Immediate; no bucket-edge crossings among continuing rows) |
| Outlook Coverage | Shared mailbox nht.cs@unisco.com **unavailable this cycle** (403 — delegated Mail.Read.Shared grant not effective); delegated inbox scanned to 07:16Z — **0 new unique eligible threads** — **10** eligible-ticket threads carried forward (3.1% of 325); 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident) |
| Last Refresh | 2026-09-08 03:20 ET (**AUTHORITATIVE** – fresh TicketOps LIVE pull ~07:16–07:22Z; server total 397 open UFN = 283 New/73 Pending/41 Reopen/0 Open re-verified via displayStatusIds [11]/[6]; **+14 arrivals** UFN-70047…70060 (SMEG USA INC — Amazon Freight LTL pickup-date & Amazon Inbound pickup-disruption notices, created 06:12–06:13Z); 0 removals / 0 status flips; eligible **325** = 256 New/69 Pending (+14 net); closeFlag still NOT a gate (19 retained, unchanged ids); ~6 retained rows newly assigned since v24 (ids not enumerated this cycle — per-row assigned carried); UFN-67030 remains Solved/closed 09/01 → outside gate) |
| Next Refresh | Hourly dashboard refresh continues; next daily summary email 2026-09-08T08:00:00-04:00 |

### ⏱ Queue Delta (this refresh: v24 → v25)

**AUTHORITATIVE v25 pull (2026-09-08 ~03:16–03:22 ET / 07:16–07:22Z)** — fresh TicketOps LIVE enumeration (dept 323826714354839552, `POST /v1/iam/tickets/page`, `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10], displayStatusIds:[11,6]}`): server total **397 open UFN** = **New 283** (displayStatusId 11 re-verified) + **Pending 73** (id 6 re-verified) + **Reopen 41** + Open 0 — **+14 vs v24** (all New). Eligible base (New+Pending) **356** = ALL 311 v24 rows still present (**0 removals**) + **14 arrivals UFN-70047…70060** (created 09/08 06:12–06:13Z; SMEG USA INC org; 9 × Amazon Freight LTL "Updated Pickup Date" + 5 × Amazon Inbound "Pickup Disruption" notices; all New/Medium/unassigned/closeFlag=false/On Track) — **0 status flips**. Audits carried unchanged: **41 Reopen** excluded (display name, outside gate); **22 billing/UF Billing/storage/handling/claims invoice items** (v24 audit ids re-verified, operational keepers retained); **9 duplicate threads** (same v24 pairs, no new twins among arrivals). → Final eligible **325 = 256 New / 0 Open / 69 Pending** (+14 net vs v24). Per-row ages advanced **+4 whole hours** from v24 anchor 2026-09-08T02:53:00Z (measured delta 4h27m; ageDays=floor(ageHours/24); **2 rows +1 age-day** — UFN-48670, UFN-69696); oldest UFN-33604 4476h→**4480h / 186d** (ageDays unchanged). Buckets recomputed **Immediate 38 / Short-Term 35 / Medium-Term 70 / Watch 182** (all 14 new arrivals <24h in Immediate; no continuing row crossed a bucket edge). SLA 200 breached / 125 on track (200 carried among 311 continuing — no membership change; +14 new rows On Track; per-id flips not separately re-tallied); unassigned 261 (server-authoritative 287 raw − 26 unassigned exclusions; ~6 retained rows newly assigned since v24, ids not enumerated — per-row assigned carried); customers 55 (tiers recomputed deterministic **37 Critical / 10 Warning / 8 Healthy** — SMEG USA INC 26→40 tickets, 16 breached, unchanged Critical). **UFN-67030** remains Solved/closed (09/01, system 20) → outside gate; its older "live-Pending" description in earlier prompts predates the 09/01 closure — the no-closeFlag-gating rule itself is unchanged and honored. **Outlook**: shared mailbox nht.cs@unisco.com **unavailable** this cycle (403 ErrorAccessDenied — grant not effective; delegated inbox nicole.weber@unisco.com scanned to 2026-09-08T07:16:33Z: 0 UFN/ops hits, internal mail only); **0 new unique eligible threads** since v24 watermark 02:44:00Z — 10 eligible-ticket threads carried unchanged (3.1% of 325); 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident). All public/data synced.
...
|---------|-----------|------------|
| refresh-2026-09-08T03:20ET-AUTHORITATIVE-v25 | 03:20 | **AUTHORITATIVE REFRESH v25** – Fresh TicketOps LIVE pull 09/08 ~03:20 ET (07:16–07:22Z; dept 323826714354839552; displayStatusIds [11]=283 / [6]=73 re-verified): **397 open UFN = 283 New/73 Pending/41 Reopen/0 Open** (+14 vs v24). Eligible base 356 = ALL 311 v24 rows (**0 removals/0 flips**) + **14 arrivals UFN-70047…70060** (SMEG USA INC; Amazon Freight LTL pickup-date ×9 + Amazon Inbound pickup-disruption ×5 notices; New/On Track/unassigned) → after unchanged audits **325 eligible = 256 New / 0 Open / 69 Pending (+14 net)**. Excluded 41 Reopen + 22 billing/storage/handling/claims (v24 audit ids re-verified; operational keepers retained) + 9 dup threads (same pairs). closeFlag still NOT a gate – 19 live `closeFlag=true` retained (unchanged ids). Ages +4 whole hours (v24 anchor 02:53Z → 03:20 ET); oldest UFN-33604 4480h/**186d**; 2 rows +1 age-day. Buckets recomputed **38/35/70/182** (14 arrivals Immediate). SLA **200/125** (200 carried + 14 new On Track); unassigned **261** (server-authoritative; ~6 retained rows newly assigned since v24, ids not enumerated); customers **55** (SMEG 26→40; tiers 37/10/8 unchanged). UFN-67030 remains Solved/closed (09/01). Outlook: shared mailbox nht.cs@unisco.com **403** (grant not effective) – 0 new unique eligible threads (02:44Z→07:16:33Z) – 10 carried (3.1%); escalations UFN-69811 + Boundless load-45872185 persist; watermark 2026-09-08T07:16:33Z. All public/data synced. |
| refresh-2026-09-07T22:53ET-AUTHORITATIVE-v24 | 22:53 | **AUTHORITATIVE REFRESH v24** – Fresh TicketOps LIVE pull 09/07 ~22:53 ET (02:53Z; dept 323826714354839552; displayStatusIds [11]=269 / [6]=73 re-verified): 383 open UFN = 269 New/73 Pending/41 Reopen — distribution IDENTICAL to v23 (newest UFN-70035 == v23 max ⇒ 0 arrivals / 0 removals / 0 status flips). Eligible base 342 → after unchanged audits **311 eligible = 242 New / 0 Open / 69 Pending (net 0)**. Excluded 41 Reopen + 22 billing/storage/handling/claims (v23 audit ids unchanged; operational keepers retained) + 9 dup threads (same pairs). closeFlag still NOT a gate – 19 live `closeFlag=true` retained. Ages +3 whole hours (v23 anchor 23:38Z → 22:53 ET); oldest UFN-33604 4476h/**186d**; 30 rows +1 age-day. Buckets recomputed **24/35/70/182** (+3h moved 17 Short→Medium, 2 Medium→Watch). SLA **200/111** (carried); unassigned 253; customers **55** (tiers 37/10/8 unchanged). UFN-67030 remains closed (09/01). Outlook: shared mailbox nht.cs@unisco.com **reachable** (delegated; prior 403 cleared) — 0 new unique eligible threads – 10 carried (3.2%); watermark 2026-09-08T02:44:00Z. All public/data synced. |
...
**This refresh (Sep 8 03:20 ET) is an AUTHORITATIVE refresh (v25)** with a fresh TicketOps LIVE connection (dept 323826714354839552, `POST /v1/iam/tickets/page` with `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10], displayStatusIds:[11,6]}`; displayStatus counts re-verified server-side via `displayStatusIds [11]`=283 and `[6]`=73). All statuses verified directly against TicketOps; the open population is **397 = 283 New / 73 Pending / 41 Reopen / 0 Open** (+14 vs v24 — 14 new arrivals UFN-70047…70060, created 09/08 06:12–06:13Z; no tickets removed/status-changed in the window; all 311 v24 eligible rows still present). The eligibility gate is `displayStatusSystemStatus=open` + `displayStatusName ∈ {New,Pending}`; `closeFlag` is not a filter (19 live `closeFlag=true` tickets retained in the eligible set — auto-close artifacts must not cause false negatives). Billing/UF Billing/storage/handling/claims invoice items and duplicate ticket/email threads are excluded per the audited rule set in `refresh-manifest.json` (22 invoice items + 9 duplicate threads, unchanged v24 audit sets).

**Queue health**: 325 verified eligible tickets (256 New / 69 Pending) — all 311 v24 rows still open (0 removals) plus 14 new arrivals (0 status flips). 200 SLA-breached / 125 on-track; 261 unassigned (server-authoritative; ~6 retained rows newly assigned since v24, ids not enumerated this cycle); oldest 186d (UFN-33604, 4480h). Stale backlog remains dominated by recurring New automated/system reports (dropship EOD, appointment-watch, Amazon EDI alerts, Advanced Report – Dock Activity) and older New tickets awaiting assignment; the 14 newest rows are SMEG USA INC Amazon Freight LTL pickup-date / Amazon Inbound pickup-disruption notices (all On Track, Immediate). Per-row ages advanced +4 whole hours from the v24 anchor (02:53Z → 03:20 ET); 2 rows advanced +1 age-day (UFN-48670, UFN-69696); action buckets recomputed to Immediate 38 / Short-Term 35 / Medium-Term 70 / Watch 182. Outlook enrichment: shared mailbox nht.cs@unisco.com unavailable this cycle (403 — delegated Mail.Read.Shared grant not effective); delegated inbox scanned to 2026-09-08T07:16:33Z with 0 UFN/ops hits; 0 new unique eligible threads since v24 watermark 02:44:00Z — 10 active eligible-ticket threads carried forward (3.1% of 325); non-blocking. Data is authoritative in `dashboard/data/` and `public/data/`.
