...
## Current Dashboard State (Last Refresh: Sep 7 06:20 ET – AUTHORITATIVE v22)

| Total Raw (open UFN, dept-gate) | 352 |
| Eligible | **287** (218 New, 0 Open, 69 Pending) |
| UFN-Count | 287 |
| Excluded | 65 — 38 Reopen display-name (outside New/Pending gate) + 22 billing/UF Billing/storage/handling/claims + 5 duplicate threads (same 5 pairs as v21: kept 69946 dropped 69943 / kept 69942 dropped 69941 / kept 69896 dropped 69893 / kept 69892 dropped 69889 / kept 69663 dropped 69661) |
...
| Priority | 287 Medium |
| SLA Risk | **ELEVATED** – 200 SLA-breached / 87 on-track (carried from v21; no state change in window) |
| Action Buckets | Immediate **17** / Short-Term **55** / Medium-Term **38** / Watch **177** (+9h age advance moved 1 row Short-Term→Medium-Term) |
| Outlook Coverage | Partial this cycle (shared mailbox 403 ErrorAccessDenied; delegated inbox scanned 09-06 21:00 ET → 09-07 06:13 ET, **0 new hits**) — **10** eligible-ticket threads carried forward (~3.5%); strongest signals Diageo/Ritual claim 18050 (UFN-69811), Boundless Walmart load-45872185 (UFN-69618/UFN-69771/UFN-69942) |
| Last Refresh | 2026-09-07 06:20 ET (**AUTHORITATIVE** – fresh TicketOps LIVE pull; server total 352 open UFN unchanged; **population identical to v21** 21:00 ET — newest open UFN-69967 (created 09-06 20:51Z) == v21 max, 0 arrivals/0 removals in ~9h19m; eligible **287 unchanged** = 218 New/69 Pending (0 net); closeFlag still NOT a gate (19 retained); UFN-67030 remains closed 09/01 → outside gate) |
...
### ⏱ Queue Delta (this refresh: v21 → v22)

**AUTHORITATIVE v22 pull (2026-09-07 ~06:20 ET / 10:20Z)** — fresh TicketOps LIVE enumeration (dept 323826714354839552, `POST /v1/iam/tickets/page`, `input{departmentIds:[323826714354839552], displayStatusSystemStatus:[10]}`, UFN prefix client-side): server total **352 open UFN** (+0 vs v21 352). **Population identical to v21 (01:00:41Z anchor)** — newest open ticket UFN-69967 (createTime 09/06/2026 20:51:44Z) equals the v21 max open/eligible id ⇒ 0 arrivals; total unchanged ⇒ 0 removals; eligible set unchanged **287 = 218 New / 69 Pending** (net 0). Eligibility gate unchanged: `displayStatusSystemStatus=open` + `displayStatusName ∈ {New,Pending}`; `closeFlag` is **NOT** a filter → **19 live `closeFlag=true` tickets retained** (UFN-68733/UFN-68750 auto-close artifacts; UFN-59720, UFN-69797 Diageo Picks, UFN-69618 Boundless no-labels, UFN-43887 Ritual stickering). Same audit exclusions — **38 Reopen** (display name), **22 billing/UF Billing/storage/handling/claims invoice items** (v21 audit ids re-verified present; operational keepers retained: UFN-60009 BOL, UFN-68749 storage-SQFT data request, UFN-69818 open-RN ops digest, UFN-69231/UFN-63762/UFN-63959 invoice-dispute ops, UFN-69811 Diageo claim 18050, Boundless incident UFN-69771/UFN-69618/UFN-69942) and **5 duplicate ticket/email threads** (same 5 pairs as v21: kept 69946 dropped 69943 / kept 69942 dropped 69941 / kept 69896 dropped 69893 / kept 69892 dropped 69889 / kept 69663 dropped 69661 — regenerated twin emails). Per-row ages advanced from the v21 exact anchor 2026-09-07T01:00:41Z by the live measured delta +9h19m → **+9 whole hours**; ageDays = floor(ageHours/24); oldest UFN-33604 4451h→4460h / **185d unchanged**; buckets recomputed **Immediate 17 / Short-Term 55 / Medium-Term 38 / Watch 177** (one row crossed Short→Medium on the age advance). SLA 200 breached / 87 on track (carried from v21 — no eligible-row state change detected in window: 0 arrivals/removals, per-row status spot-checks on the newest page match v21, due dates remain future); unassigned 231; customers 53 (unchanged ticket-visible set; roster supplemental; tier split unchanged 36 Critical / 6 Warning / 11 Healthy). **UFN-67030** remains outside the open set (closed 09/01) → outside gate. **Outlook**: delegated mailbox accessible via Graph, shared mailbox nht.cs@unisco.com still 403 ErrorAccessDenied; window 09-06 21:00 ET → 09-07 06:13 ET (01:00:41Z→10:13Z): **0 new UFN/Cesanek hits** (one Teams @everyone digest without ticket mapping not counted) — 10 eligible-ticket threads carried forward unchanged (3.5% of 287); 2 eligible escalations persist (UFN-69811 Diageo/Ritual claim 18050; Boundless Walmart load-45872185 as one incident). Watermark advanced to 2026-09-07T10:13:00Z (06:13 ET).
